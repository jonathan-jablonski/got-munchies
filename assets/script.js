...
function restaurantsByCategory() {
  ...
  $.ajax({
    dataType: "json",
    url: `https://api.foursquare.com/v2/venues/explore?client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=20180323&categoryId=${foodCategory.join(
      ","
    )}&limit=10&ll=${location}&radius=${radius}`,
    success: function (data) {
      ...
      L.mapquest
        .geocoding()
        .geocode(
          restaurants[0].venue.location.formattedAddress.join(","),
    ...
    success: function (data) {
     ...
     var randomRestaurant = restaurants[randomNumber];
      L.mapquest
        .geocoding()
        .geocode(
          randomRestaurant.venue.location.formattedAddress.join(","),
          function (error, result) {
            console.log(result);
            currentLocation = result.results[0].locations[0].latLng;
            console.log(currentLocation);
            map.setView(currentLocation);
            $('.modal-card-body').html(`
            <div class="restaurant-name">${randomRestaurant.venue.name}</div>
           `);
            $(".modal").addClass("is-active")
          }
        );
